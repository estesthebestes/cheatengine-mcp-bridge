...
local function toHex(num)
    ...
    -- Lua 5.3+ integers are 64-bit signed; string.format("%X") handles them correctly
    -- including kernel-space addresses that appear negative as signed int64.
    return string.format("0x%X", num)
end

local function toHexLow32(num)
    if not num then return "nil" end
    return string.format("0x%08X", num % 0x100000000)
end
    ...
    if okLog and log then
        ...
        for i = 1, count do
            ...
            local hitData = {
                ...
                ESP = entry.RSP and toHexLow32(entry.RSP) or nil,  -- Lower 32 bits for 32-bit game
                RSP = entry.RSP and toHex(entry.RSP) or nil,
                EIP = entry.RIP and toHexLow32(entry.RIP) or nil,  -- Lower 32 bits
